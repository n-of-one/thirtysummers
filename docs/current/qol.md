...
1. **A fixed view.** The game is drawn in a logical view of
   ...
   width, so the ring keeps its shape at any size. `?view=1920x1080`
   overrides the size for the experiment. The size to keep is chosen in
   play: 1280×720 first, against 1920×1080 and 1600×900, and against
   `TILE 48` if 20 × 11 tiles feels cramped. The choice goes into config and
   into architecture.md's fixed decisions.
2. **Tools aim where the player last walked.** `player.heading` is the last
   direction the input asked for, eight ways, kept while standing still and
   empty until the first step. Cutting and building still take a tile within
   `INTERACT_RADIUS` of the player, but among those they take the one
   nearest to the point `AIM_AHEAD_TILES` ahead along the heading, ties on
   the lower index as now. In the middle of a bridge tile the next tile
   along sits on the aim point and the ones beside it a tile away, so it
   wins outright and the hold no longer restarts as the player shifts.
...
3. **The last minute.** `HOMEWARD_SEC` replaces `CLOCK_URGENT_SEC`. Below
   ...
   thirst one under the player: "Summer ends in 0:47. Get back to camp." At
   camp the End summer button is already there, so nothing is said. An arrow
...
6. **Pause.** `P` pauses: nothing steps, the camera and the water hold, and
   ...
   since a five-minute clock and a doorbell do not mix. It resumes on `P`,
   not on focus, so coming back to the window is not the same as being
   ready. The page title becomes "thirtysummers".
...
- On a bridge tile with the heading west, the marker is on the tile ahead at
  every position across the tile, walked, not teleported. With the aim
  removed the marker flips at the tile's centre.
- At `HOMEWARD_SEC` left and away from camp, the notice shows the clock's
  ...
  shows.
- The dusk layer is at zero opacity a second before `HOMEWARD_SEC` and at
  `DUSK_MAX_ALPHA` at the end, and a screenshot in the last ten seconds is
  darker and warmer in its mean pixel than one a minute earlier, with the
  fog held at full hydration so the fog is not what is measured.
...
- Ten seconds paused leave the clock where it was, and a blur pauses.
